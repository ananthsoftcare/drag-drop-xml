...
export const generateFileName = (type, message_id, order_id) => {
	...
	} else if (type === config.xmlType.WAREHOUSE) {
		return `CS_Warehouse_${date}_${time}_${message_id}_${order_id}`;
	} else if (type === config.xmlType.PACKINGSLIP) {
		return `CS_PackingSlip_${date}_${time}_${message_id}_${order_id}`;
	}
